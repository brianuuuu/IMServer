...
/**********************
 * class flex_string;
 * class CCowNewStringStorage;
 */

template <typename E> struct CWbxHTraits;

template <> struct CWbxHTraits<char>
{
...
public:
	...
	CCowNewStringStorage(const CCowNewStringStorage& s)
		: m_pData(s.m_pData)
	{
		AddRef(m_pData);
	}

	CCowNewStringStorage(const A&)
		: m_pData(const_cast<Data*>(s_pEmptyString))
    {
	}

	CCowNewStringStorage(const E* s, size_type len, const A&)
		: m_pData(const_cast<Data*>(s_pEmptyString))
	{
		InitAlloc(len);
	...
	bool modifier_optimized(size_type pos1, size_type n1, 
		const CCowNewStringStorage& another, size_type pos2, size_type n2)
	{
		bool ret = false;
...
template <typename E, class A> const CCowNewStringStorage<E, A>::Data * 
CCowNewStringStorage<E, A>::s_pEmptyString = (const CCowNewStringStorage<E, A>::Data*)&s_InitEmptyData;
...
//#define Enforce(c, e, m) WBXH_ASSERT_AND_THROW(c, m) 
